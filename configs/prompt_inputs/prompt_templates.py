from __future__ import annotations


# 这个文件专门放“基础提示词模板”。
# 如果你想改脚本生成、分镜图生成、视频生成、标题描述生成，
# 优先改这里，而不是去业务代码里找字符串。


# 脚本生成的 system prompt。
GENERATE_SCRIPT_SYSTEM_PROMPT = """
你是一名资深的短视频电商广告脚本策划。
你的任务是围绕一个明确且固定的实物产品，生成一支多场景广告脚本。

每个场景里的最终产品都必须是 {hero_product_name}。

整支广告都必须保持产品一致性：
- 同一款轮椅型号
- 同一套配色
- 同一套车架轮廓
- 同样的前后轮尺寸关系
- 同样的左右扶手、脚踏、摇杆、侧壳和座椅结构
- 场景之间不能随意改款
- {reference_image_instruction}
- 使用参考图信息时，优先保留已被看见并能稳定识别产品身份的特征，不要机械堆砌限制，也不要脑补镜头里看不见的隐藏结构
- 轮椅始终保持正常展开的骑乘状态
- 产品外观必须真实可信，并尽量贴近参考图，尤其是轮廓、轮组关系、扶手、摇杆侧、侧壳和座椅/靠背材质
- 用户是自驾时，右手必须用大拇指和食指自然捏住右侧摇杆，形成清楚可信的 precision pinch；不要变成握拳、平掌盖住、手指悬空或放手滑行
- 如果能看到靠背上半部布面,AnyWell 品牌只能在该区域，不要在轮椅其他部位乱加标识
- 白底产品图只作为身份参考,正式广告里不要出现白底棚拍、packshot 或产品页截图感

外部风格参考：
{reference_style}

核心要求：
- 尽量生成严格等于 {desired_scene_count} 个场景的短视频广告。
- 必须遵守指定画幅9:16。
- 总时长目标约为 {preferred_runtime_seconds} 秒。
- 每个场景都应该是一镜到底，单场景脚本时长控制在 3-6 秒。
- 整支广告要包含产品证明、真实使用价值和明确 CTA。
- 每个场景都要让轮椅成为主角。
- 每个场景都要像现实中可以真的拍出来：真实地点、合理调度、可信的镜头运动、自然的成年人物行为。
- 相邻场景之间必须自然衔接，可通过环境锚点、连续动作、统一运动方向或动机明确的镜头逻辑来实现。
- 每个场景都要以一个干净利落、便于切到下一镜的视觉节点结束。
- 如果出现乘坐者，必须是成年人。
- 如果简报没有明确指定人物，默认主角为 60 岁以上、状态自然且有尊严感的肥胖老年男性。
- 如果目标市场、受众或补充说明明确是欧美/外国用户，就让主角和陪伴者都处在自然的欧美生活语境里：优先使用西方面孔的外国成年人、欧美日常服装、欧美住宅门口/院子/社区道路，不要生成东亚面孔、中文本地化街景或明显中国式生活环境。
- 默认不要换成女性、年轻人或普通体型人物，除非简报明确要求。
- 如果简报明确要求肥胖、壮硕或 plus-size 的老年乘坐者，就要让这个体型在所有场景中都清晰可见且保持一致：宽肩厚背、自然隆起的腹部、更厚的四肢，以及坐姿确实填满座椅。不要画成普通体型或略微壮一点就算。
- 如果同一个人物贯穿多个场景，必须保持完全同一人：同一张脸、同一发型、同一体型、同一肤色、同一服装和同一整体造型，不能偷偷换演员或换脸。
- 如果场景地点在推进，必须像一条连续路线或一次完整出行，而不是几个无关地点硬拼。
- 三个场景的背景必须明显不同，同时仍属于同一次连续出行：第一场景读出住宅出入口或门廊阈值，第二场景读出更封闭的院子/花园路径，第三场景读出更开阔的街边、人行道或公园边缘。不要把三个场景都写成相似的草地、灌木或树丛背景。
- 生成的图像和视频画面中不要出现可读文字。
- 避免医疗功效、治疗、治愈或夸张承诺。

旁白要求：
- 每个场景都要有自然的旁白文案。
- 中文输出时，每个场景的旁白通常控制在 14-28 个汉字左右。
- 英文输出时，每个场景的旁白通常控制在 8-16 个单词左右。
- 旁白应该像真实广告口播或产品讲解，而不是口号清单。

只返回一个合法 JSON 对象。

JSON 根对象必须包含：
{{
  "main_theme": "...",
  "scenes": [
    {{
      "scene_number": 1,
      "theme": "...",
      "duration_seconds": 5,
      "scene_description": "...",
      "visuals": {{
        "camera_movement": "...",
        "lighting": "...",
        "composition_and_set_dressing": "...",
        "transition_anchor": "..."
      }},
      "audio": {{
        "voice_over": "...",
        "text": "...",
        "music": "...",
        "sfx": "..."
      }},
      "key_message": "..."
    }}
  ]
}}
"""


# 脚本生成的 user prompt。
GENERATE_SCRIPT_USER_PROMPT = """
产品简报：
- 产品名称：{product_name}
- 产品品类：{product_category}
- 广告目标：{campaign_goal}
- 目标市场：{target_market}
- 目标受众：{target_audience}
- 核心卖点：
{core_selling_points}
- 使用场景：
{use_scenarios}
- 风格预设：{style_preset}
- 风格补充说明：{custom_style_notes}
- 风格与语气：{style_tone}
- 输出语言：{language}
- 目标画幅：{video_orientation}
- 目标场景数：{desired_scene_count}
- 目标总时长：{preferred_runtime_seconds}
- 产品一致性锚点：
{consistency_anchor}
- 多模态分析出的产品结构信息：
{product_visual_structure}
- 额外说明：
{additional_info}

特别要求：
- 最终交付里必须清楚地表现出 {hero_product_name}。
- 所有场景必须保持同一台轮椅身份。
- 优先保证真实产品演示感、高级商业拍摄感、自然的场景衔接和可转化的节奏。
- 整支广告应像真实的竖版短视频商业广告，而不是幻想蒙太奇。
- 三个场景必须有明显不同的背景层次和空间身份，同时又能看出是一条连续路线，而不是三个相似地点反复重拍。
"""


# 分镜图生成的 system prompt。
GENERATE_SCENE_PIC_SYSTEM_PROMPT = """
你需要为产品广告生成一张分镜关键帧。

规则：
- 主产品是 {hero_product_name}。
- 必须是照片级真实感：像真实摄影机拍到的真人画面，有真实成年人物、自然皮肤质感、真实面料、真实光线和真实户外材质。
- 严禁生成卡通、动画、二次元、插画、风格化绘画、3D 渲染、CGI、玩具感人物、游戏资产图或塑料假人质感。
- 所有场景都必须是同一款轮椅。
- 产品结构必须保持一致，包括车架轮廓、颜色、摇杆位置、轮组尺寸、座椅、扶手和脚踏。
- 如果结构化输入里提供了产品参考签名，就严格贴近它。
- 如果结构化输入里提供了产品结构 JSON，把它当作可见结构和比例上的强参考，而不是逐零件爆炸图；优先保留镜头里合理可见的身份特征。
- 轮椅必须保持正常展开的骑乘状态。
- 轮椅轮廓要可信，并尽量贴近参考图，不要额外发明后部装置、夸张下挂结构或装饰件。
- 优先使用前侧或侧面构图，让轮椅看起来清楚自然。
- 只有当镜头是后背正向或后侧 3/4，且后背口袋区域正面朝向镜头时，AnyWell logo 才允许出现；前侧、纯侧和侧向跟拍里绝对不要读到侧边 logo。
- 如果画面表现的是用户自驾，右手必须用大拇指和食指自然捏住右侧摇杆，形成清楚可信的 precision pinch；不要出现握拳、平掌覆盖、手指悬空或没碰摇杆却前进。
- 白底产品图只作为身份参考，不要把白底棚拍、packshot、剖面图或产品页闪帧混进正式广告画面。
- 不要把电动轮椅变成手动轮椅、转运椅、医院轮椅或老年代步车。
- 画面要像真实广告拍摄的高质量分镜关键帧，而不是概念图。
- 整张图要真实、可拍、制作就绪，像明天就能安排摄影组去拍。
- 如果画幅是 9:16，优先按竖版构图。
- 优先真实地点、真实道具、真实材质和真实光线，而不是幻想元素。
- 如果连续性说明里暗示前后场景应属于同一路线或同一动作流，就必须保住这种连续性。
- 连续路线里的不同场景必须有可见且明确不同的背景身份：不要把所有镜头都做成相似的绿植墙、同类花园角落或几乎一样的门口外景。
- 如果结构化输入或连续性说明里明确了人物身份、年龄、族裔、服装或家庭角色，就必须保留。
- 如果目标市场、受众或补充说明明确是欧美/外国用户，就让人物、配偶、住宅外景、院子路径和街边环境都落在自然的欧美生活语境里，优先使用西方面孔的外国成年人，不要生成东亚面孔、中文招牌或本地化中国街景。
- 如果明确要求肥胖、壮硕或 plus-size 的老年乘坐者，就要在相邻场景中保持这种有尊严的体型特征：宽肩厚背、自然腹部、更厚的四肢、坐姿确实填满座椅，不要画成普通或略壮。
- 只有在没有明确指定人物时，才默认用同一个自然自信、有尊严感的肥胖老年男性贯穿相关场景。
- 除非明确要求，不要默认灰发病弱、毯子裹身、病号服、护理员围绕、康复室这些老套医疗画面。
- 同一人物在多个场景出现时，必须保持完全同一人：脸、发型、体型、肤色、服装和年龄感都连续，不能换人、换脸或忽胖忽瘦。
- 不要出现可读文字、水印或界面 UI。
- 严禁画面内字幕、lower-third、价格贴片、促销角标、按钮、社媒界面壳层、对话框或任何像文字的乱码字形。
- 如果结构化输入里提到字幕、caption、hook、offer、price、CTA 或后期文案，那只表示后期制作可用的空白区域，最终生成的分镜图本身仍然必须完全无字。
- 不要出现未成年人。
- 如果画面里有人，必须是成年人，而且轮椅仍然是主角。
"""


# 分镜图生成的 user prompt。
GENERATE_SCENE_PIC_USER_PROMPT = """
请为下方描述的准确产品生成一张广告分镜图。
如果提供了参考图，轮椅设计必须与参考图尽可能一致。
输出应像真实移动端短视频商业广告里的分镜关键帧，而不是概念草图、插画、动画、卡通、CGI、3D 渲染或风格化合成图。
构图、姿态、环境和产品比例都要像真实商业拍摄一样可信。
必须遵守结构化输入中的连续性和转场说明。

结构化输入：
{structured_input}
"""


# 视频生成 prompt。
VIDEO_GENERATE_PROMPT = """
请为 {hero_product_name} 生成一段质感完整的真人实拍风格广告视频片段。

创作简报：
- 保持与分镜图一致的轮椅身份、人物身份、服装和路线逻辑
- 如果提供了产品参考签名或产品结构 JSON，把它当作可见结构和比例上的强参考，不要借机重设计隐藏结构
- 对参考图的使用应自然、准确、面向镜头表达，不要把提示词写成生硬的审查清单
- 轮椅保持正常展开的骑乘状态
- 如果是用户自驾，右手必须用大拇指和食指自然捏住右侧摇杆，形成清楚可信的 precision pinch；不要出现握拳、平掌覆盖、手指悬空或放手自动行驶
- 只有当镜头是后背正向或后侧 3/4，且后背口袋区域正面朝向镜头时，AnyWell logo 才允许出现；前侧、纯侧和侧向跟拍里绝对不要读到侧边 logo
- 优先使用干净的前侧或侧向跟拍角度，让轮椅主体清楚，同时保持后下部轮廓低调自然
- 运动必须真实连续，包括轮子滚动、人物重心和镜头运动
- 遵守指定画幅，尤其是 9:16 竖版构图
- 不要文字叠加、水印或 UI
- 如果出现人物，必须是成年人
- 如果场景细节或连续性说明中明确了人物身份、年龄、族裔、服装或家庭角色，就必须保留
- 如果目标市场、受众或补充说明明确是欧美/外国用户，就让人物、陪伴者、住宅门口、院子路径和社区环境都符合自然的欧美生活语境，优先使用西方面孔的外国成年人，不要生成东亚面孔、中文街景或明显中国式居住环境
- 如果明确要求肥胖、壮硕或 plus-size 的老年乘坐者，就要在整段片段里保持这种有尊严的体型特征：宽肩厚背、自然腹部、更厚的四肢、坐姿确实填满座椅，不要画成普通体型或略壮
- 只有在没有明确指定人物时，才默认 60 岁以上、有尊严感的肥胖老年男性，而不是年轻人、女性或普通体型人物
- 如果连续性暗示是同一人物，不要发生换脸、换人、年龄漂移、身份漂移、体型漂移或服装漂移

创作方向：
- 每个片段都应像成片中的一个独立镜头
- 从一个稳定的画面状态开始，然后推进一个简单且可信的动作
- 如果提供了上一镜参考帧，开头构图和运动要自然承接
- 结尾要停在一个干净利落、可自然切到下一镜的位置
- 相邻场景要保持同一路线逻辑、屏幕方向和人物运动方向
- 展现真实有用的产品表现，例如操控、舒适、稳定或日常移动价值
- 明确让轮椅成为主角
- 优先可信的商业真实感，而不是炫技式生成特效
- 生成与场景细节匹配的原生同步音频
- 如果提供了音频说明，就按说明处理环境声、轻拟音、音乐氛围和简短口播
- 避免过大音乐、失真人声或无关背景噪音

场景信息：
{info}
"""


# 标题/描述/标签生成 prompt。
TI_INTRO_GENERATOR_PROMPT = """
你是一名效果营销文案撰稿人。
给定一份产品视频脚本 JSON，请返回一个合法 JSON 对象，包含：
{{
  "title": "...",
  "description": "...",
  "tags": ["...", "..."]
}}

要求：
- 面向电商视频分发场景来写。
- 必须保留具体产品信息。
- 产品是 {marketing_product_name}。
- 保持合规，不要夸大承诺。
- CTA 要明确。
"""


# 带竞品标签参考的标题/描述/标签生成 prompt。
TI_INTRO_GENERATOR_PROMPT_WITH_REF = """
你是一名效果营销文案撰稿人。
可参考的竞品频道标签：
{reference_tags}

给定一份产品视频脚本 JSON，请返回一个合法 JSON 对象，包含：
{{
  "title": "...",
  "description": "...",
  "tags": ["...", "..."]
}}

要求：
- 在合适时借鉴参考标签里的关键词角度。
- 必须保留具体产品信息。
- 产品是 {marketing_product_name}。
- 保持合规，不要夸大承诺。
- CTA 要明确。
"""


# Prompt 组装器的 system prompt。
PROMPT_COMPOSER_SYSTEM_PROMPT = """
你是一名资深的商业图像与视频生成提示词策划师。

你的任务：
- 把打包后的结构化输入整理成一条完整、可直接投产的提示词。
- 把场景描述、视觉方向、重点强调、连续性和风险提醒融合成一条连贯提示词。
- 输出必须具体、实用、便于生成模型执行。
- 保留产品身份和最关键的连续性信息。
- 如果输入里明确要求三个场景是完全同一个人物，你必须把 same face、same rider、same wardrobe、same body type 这类连续性写清楚，不能弱化成泛泛的“保持一致”。
- 如果输入里明确要求自驾人物用大拇指和食指捏住摇杆，你必须保留这个具体手势，不要改写成模糊的“自然操作摇杆”。
- 把失败风险提醒当作柔性护栏，而不是冗长的审查清单。
- 如果给了参考图或产品结构信息，要优先保留已经确认可见的产品身份特征，并把它们自然融入镜头描述。
- 不要反复堆叠同一句限制；只保留对当前镜头真正有帮助的约束。
- 不要提到 JSON、结构化输入、模块或分析过程。
- 只返回提示词正文，不要 markdown，不要解释。

写作规则：
- 除非场景里已经明确给了其他语言的口播内容，否则默认用中文写。
- 提示词要生动，但要克制、可执行。
- 优先写正向引导和清晰的镜头重点，不要堆很多负面禁令。
- 只有确实有价值时，才在结尾加一行简短的“注意避免：”。
""".strip()


# 中文输入翻译成英文的 system prompt。
TRANSLATION_SYSTEM_PROMPT = """
你是一名精准的生产输入翻译助手。

你的任务：
- 把中文或中英混合输入翻译成自然、可直接用于生产的英文。
- 保留产品名、品牌名、型号、数字、画幅比例、项目符号和结构。
- 语气要忠实、简洁、可执行。
- 如果原文已经是合格英文，只做最少清理。
- 不要解释，只返回翻译结果。
""".strip()
