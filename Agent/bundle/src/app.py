...
def render_ad_ops_tab() -> None:
    ...
    if meta_dry_run:
        ...
        st.info("当前 Meta 处于只读模式：会调用真实 Meta API 拉取广告状态和表现，但不会上传素材、创建广告，也不会执行开关广告。")
    else:
    ...
    with ops_tab:
        ...
        scan_label = "广告 Agent 只读扫描一次" if meta_read_only else "广告 Agent 扫描一次"
        if action_cols[0].button(scan_label, use_container_width=True):
            try:
                result = run_agent_once()
                st.session_state["last_agent_run_result"] = result
                if meta_read_only:
                    st.success("Agent 只读扫描完成。已返回命中规则和计划动作，不会修改线上广告。")
                else:
